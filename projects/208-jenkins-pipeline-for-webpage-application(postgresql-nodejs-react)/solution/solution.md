...
- configure jenkins server. (install suggested plugins > Create First Admin User > Save and Finish > Start Jenkins)
...
- Follow these steps `manage jenkins > manage plugins > available > add ansible and terraform plugins (locale plaugin if it is necessary) - gloabal tool configuration`.

```yml
- For english language:

  - manage jenkins > configure system > Locale : write `en` and click the box below > save and apply.
...
- Click Add Credentials on the left.
  ...
  Username: project          # Github personel access token name
  Password: <github token>   # Github personel access token

- Click Add Credentials on the left, again. # now we will define our pem file for ansible connection to managed nodes.

  Kind: SSH Username with private key
  ID: clarus                 # your pem file name
  Description: ansible       # Description for private key
  Username: ec2-user         # username for ssh connection to managed nodes
  Private Key:
    - click `Enter directly`
    - click `Add`  # copy your pem file content from your local to here.
    - click `OK`.
...
### Run ansible playbook with `Pipeline Syntax`

```yml
- Snippet Generator > 
Steps:
  Sample Step: ansiblePlaybook: Invoke an ansible playbook
Playbook file path in workspace: docker_project.yml
Inventory file path in workspace: inventory_aws_ec2.yml
SSH connection credentials: select `ec2-user (ansible)`
- click the box `Disable the host SSH key check` 
- click `Generate Pipeline Script`

output: ansiblePlaybook credentialsId: 'oliver', disableHostKeyChecking: true, installation: 'ansible', inventory: 'inventory_aws_ec2.yml', playbook: 'docker_project.yml'
```
